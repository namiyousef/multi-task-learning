...
class RandomHeadNoLayers(nn.Sequential):

    def __init(self):
        super(RandomHead, self).__init__()
    
    def forward(self, xOG, inputs, skips):
        #save_image(xOG/255, 'imgOG.png')
        #print(f"xOGsize {xOG[0].size()}")
        #x = nn.ConvTranspose2d(3,1,1)(xOG)
        x = xOG[:,0:1,:,:]
        #save_image(x/255, 'RNLimg1.png')
        #x = trans.RandomResizedCrop(256)(x)
        #save_image(x/255, 'RNLimg2.png')
        #print(f" xshape {x.shape}")
        #save_image(x/255, 'RNLimg3.png')
        dim = 2
        idx = torch.randperm(x.shape[dim])
        x = x[:,:,idx,:]
        #x = x[torch.randperm(x.size()[0])]
        #save_image(x/255, 'RNLimg4.png')
        #print(f"xsize {x.size()}")
        #x = x[:,torch.randperm(x.size()[1])]
        dim = 3
        idx = torch.randperm(x.shape[dim])
        x = x[:,:,:,idx]
        #save_image(x/255, 'RNLimg5.png')
        return x/255
...
class SegmentationHead(nn.Module):
    ...
    def forward(self, xOG, x, skips):
        ...
        save_image(output, 'outputSeg.png')
        return output  
